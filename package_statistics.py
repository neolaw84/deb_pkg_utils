...
ARCHITECTURES = ARCHITECTURES + ['udeb-' + x for x in ARCHITECTURES]
DEFAULT_URL = "http://ftp.uk.debian.org/debian/dists/stable/main/"
logger = None


def init_logger(module_name: str = THIS_FILE_NAME,
...
def contents_generator(
        mirror_url: str = 'http://ftp.uk.debian.org/debian/dists/stable/main/',
        file_name: str = 'Contents-amd64.gz'
) -> str:
    try:
        req = urllib.request.Request(mirror_url + file_name)
        res = urllib.request.urlopen(req)
    except urllib.error.HTTPError:
        error = "Unable to retrieve file_name: {} from {}." \
            .format(file_name, mirror_url)
        logger.error(error)
...
class KeyCounter(object):
    ...
    def add_key(self, key: str) -> None:
        ...
        logger.debug("current {key} -> counter {counter}"
                     .format(key=key, counter=counter))

    def sort_by_value(self, reverse: bool = False) -> List[Tuple]:
        return sorted(
            self.key_to_counter.items(),
            key=lambda kv: (kv[1], kv[0]), reverse=reverse
        )
...
@plac.annotations(
    architecture=("The architecture to retrieve",
                  "positional", None, str, ARCHITECTURES
                  ),
    mirror_url=(
            "The mirror url to be used (default is '{}'".format(DEFAULT_URL),
            "option", "u", str, None
    ),
    k=("Top k usage (default is 10)", "option", "k", int, None)
)
def main(
        architecture: str = None, mirror_url: str = DEFAULT_URL, k: int = 10
) -> None:
    init_logger()
    ...
    i = 0
    divider = 100
    for line in contents_generator(
            mirror_url=mirror_url,
            file_name="Contents-{}.gz".format(architecture)
    ):
        repo = get_repo(line)
        if repo:
